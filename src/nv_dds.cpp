...
void CDDSImage::upload_texture2D(uint32_t imageIndex, uint32_t target) {
	...
	if (is_compressed()) {
		...
		while ((err = glGetError()) != GL_NO_ERROR) {
			printf("glCompressedTexImage2D(1): OpenGL error: %d\n", err);
		}
		// load all mipmaps
		for (unsigned int i = 0; i < image.get_num_mipmaps(); i++) {
			...
			while ((err = glGetError()) != GL_NO_ERROR) {
				printf("glCompressedTexImage2D(2): OpenGL error: %d\n", err);
			}
		}
	}
	else {
		...
		if (!is_dword_aligned()) {
			...
			glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
			while ((err = glGetError()) != GL_NO_ERROR) {
				printf("glGetIntegerv: OpenGL error: %d\n", err);
			}
		}
		...
		//width = 1024;
		while ((err = glGetError()) != GL_NO_ERROR) {
			printf("glTexImage2D(0): OpenGL error: %d, values: %d, %d, format: %d, components: %d, target: %d\n", err, width, height, m_format, m_components, target);
		}
		//glTexImage2D(target, 0, m_components, width, height, 0, m_format, GL_UNSIGNED_BYTE, image);
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_BGRA, GL_UNSIGNED_BYTE, image);
		while ((err = glGetError()) != GL_NO_ERROR) {
			printf("glTexImage2D(1): OpenGL error: %d, values: %d, %d, format: %d, components: %d, target: %d\n", err, width, height, m_format, m_components, target);
		}
		...
		for (unsigned int i = 0; i < image.get_num_mipmaps(); i++) {
			...
			while ((err = glGetError()) != GL_NO_ERROR) {
				printf("glTexImage2D(2): OpenGL error: %d\n", err);
			}
		}
		...
		if (alignment != -1) {
			glPixelStorei(GL_UNPACK_ALIGNMENT, alignment);
			while ((err = glGetError()) != GL_NO_ERROR) {
				printf("glPixelStorei(1): OpenGL error: %d\n", err);
			}
		}
	...
	while ((err = glGetError()) != GL_NO_ERROR) {
		printf("CDDSImage:upload_texture2D: OpenGL error: %d\n", err);
	}
}
